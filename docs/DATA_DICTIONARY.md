...
## Update 2026-07-17 — repo-provenance dates for school/context data sources

`vintage_as_of` (in `data_dictionary_schools_org_enriched.csv`) is about when the real-world
data was *measured* (e.g. CRDC "School Year 2021-22") — for most sources that's still "not
confirmed in repo," waiting on Sheng/Bob. That's a different question from **when we pulled
each file into this repo**, which we can always answer ourselves from git history, no one
else's confirmation needed. `docs/data_source_provenance.csv` (generated by
`etl/data_provenance.py`, rerun any time a source is refreshed) answers exactly that, for the
school/context data sources (NCES, Census, NAEP, ISBE, IB, College Board, CPS) — 22 of 23
files have a real first/last-commit date from git.

**One gap this surfaced that wasn't previously flagged**: Sheng's combined schools export
(`data/updated-sheng/schools_combined_enriched_ceeb.csv`) is gitignored and was never
committed, so git has no record of when we received it — and since **CRDC data arrives
bundled inside that file** rather than being loaded independently by this pipeline, CRDC's
freshness can't be dated from repo history either. Same limitation this project has already
documented for Bob's org export, just not previously noticed for CRDC specifically.

Bob's NU org export has its own separate vintage note (`NU_EXPORT_VINTAGE` in
`build_modeling_dataset.py`, from the export filename's timestamp) and isn't duplicated here.
